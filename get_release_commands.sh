...
if [[ "$1" = "-h" ]] || [[ "$1" = "--help" ]]; then
  ...
  echo "Prints out commands to run to release zt-process-killer to Maven Central."
...
echo "# To release zt-process-killer version ${version} run these commands:"
echo ""
...
echo "git commit -m \"Prepare release zt-process-killer-${version}\""
echo "git tag zt-process-killer-${version}"
...
echo "mvn gpg:sign-and-deploy-file -Durl=https://oss.sonatype.org/service/local/staging/deploy/maven2/ -DrepositoryId=sonatype-nexus-staging -DpomFile=pom.xml -Dfile=target/zt-process-killer-$1.jar"

echo ""
echo "# 7) deploy and sign sources archive"
echo "mvn gpg:sign-and-deploy-file -Durl=https://oss.sonatype.org/service/local/staging/deploy/maven2/ -DrepositoryId=sonatype-nexus-staging -DpomFile=pom.xml -Dfile=target/zt-process-killer-$1-sources.jar -Dclassifier=sources"

echo ""
echo "# 8) deploy and sign javadoc archive"
echo "mvn gpg:sign-and-deploy-file -Durl=https://oss.sonatype.org/service/local/staging/deploy/maven2/ -DrepositoryId=sonatype-nexus-staging -DpomFile=pom.xml -Dfile=target/zt-process-killer-$1-javadoc.jar -Dclassifier=javadoc"
